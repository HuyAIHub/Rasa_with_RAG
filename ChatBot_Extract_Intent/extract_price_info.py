...
import pandas as pd
import json
from ChatBot_Extract_Intent.module.search_product import product_seeking_terms,product_seeking
            ...
            if data['GROUP_PRODUCT_NAME'][row].lower() in name_product and (row == 0 or data['GROUP_PRODUCT_NAME'][row-1] != data['GROUP_PRODUCT_NAME'][row]):
                ...
                for row_2 in range(row, data.shape[0]):
                    if data['GROUP_PRODUCT_NAME'][row_2].lower() == data['GROUP_PRODUCT_NAME'][row].lower():
                        a.append([data['PRODUCT_INFO_ID'][row_2], data['PRODUCT_NAME'][row_2], data['VAT_PRICE_3'][row_2], data['COMMISSION_3'][row_2]])
...
# "time_processing":''
# }
# print(take_product("Tôi muốn mua 3 đèn năng lượng mặt trời tầm giá từ 5 triệu đến 10 triệu"))
# print(time.time()-t1)
